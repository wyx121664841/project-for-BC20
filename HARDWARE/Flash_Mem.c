...
int Flash_Write(uint32_t iAddress, uint8_t *buf, uint32_t iNbrToWrite)
{
    ...
    while( 1 )
    {
        Flash_Read(secpos, flash_tmp, FLASH_PAGE_SIZE); //读出整个扇区
        for(i=0; i<secremain; i++)
        {   //校验数据
            if(flash_tmp[secoff+i]!=0XFF)break; //需要擦除
        }
        if(i<secremain)
        {   //需要擦除
            ...
            Flash_Write_Without_check(secpos,flash_tmp,FLASH_PAGE_SIZE);  //写入整个扇区
        }
        else
        {
